...
const ClientsUI = () => {
  ...
  const handleDelete = async (clientId: string) => {
    setIsLoading(true);
    setError(null); // إعادة تعيين الخطأ قبل المحاولة

    try {
      // إرسال طلب الحذف إلى نقطة نهاية API
      ...
      if (response.ok) {
        ...
        // جلب قائمة العملاء المحدثة بعد الحذف
        await fetchClients();
      } else {
        // إذا كان هناك خطأ، قم بعرض رسالة الخطأ من الخادم
        setError(data.message || "فشل في عملية الحذف.");
      }
    } catch (err) {
      console.error("Deletion error:", err);
      // التعامل مع أي أخطاء في الشبكة
      setError("حدث خطأ أثناء الاتصال بالخادم للحذف.");
    } finally {
      setIsLoading(false);
      // إغلاق نافذة الحذف المنبثقة
  ...
  const openDeleteModal = (clientId: any, clientName: any) => {
  ...
  // تصفية الموكلين بناءً على شريط البحث
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between mb-8 space-y-4 sm:space-y-0">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-2">
              إدارة قاعدة بيانات موكليك بسهولة
            </p>
          </div>
          <div className="flex items-center space-x-3 space-x-reverse w-full sm:w-auto">
            <div className="relative flex-grow">
              <input
                ...
                className="p-3 rounded-xl bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200 placeholder-gray-500 dark:placeholder-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-colors pr-12 w-full"
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
              />
              <FaSearch className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500 dark:text-gray-400" />
            </div>
            <a href="/dashboard/clients/add-client">
              <button className="bg-indigo-600 hover:bg-indigo-700 dark:bg-indigo-500 dark:hover:bg-indigo-400 text-white font-bold py-3 px-5 rounded-xl shadow-md transition-transform transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 flex items-center">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium relative">
                          ...
                          {openDropdownId === client._id && (
                            ...
                            >
                              ...
                              <button
                                onClick={() =>
                                  openDeleteModal(client._id, client.name)
                                }
